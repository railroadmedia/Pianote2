...
export async function getNewContent(type) {
    try {
        if(type == '') {type = 'course&included_types[]=song&included_types[]=unit&included_types[]=quick-tips&included_types[]=question-and-answer&included_types[]=student-review&included_types[]=boot-camps&included_types[]=chords-and-scales&included_types[]=pack&included_types[]=podcasts'}
...
export async function getStartedContent(type) {
    try {
        if(type == '') {type = 'unit&included_types[]=unit-part&included_types[]=course&included_types[]=song&included_types[]=quick-tips&included_types[]=question-and-answer&included_types[]=student-review&included_types[]=boot-camps&included_types[]=chord-and-scale&included_types[]=podcasts&included_types[]=pack-bundle-lesson'}
...
export async function getAllContent(type, sort, page, filtersDict) {
    ...
    if(type !== '') {type = `included_types[]=${type}&`} 
    ...
    try {
        let auth = await getToken();
        let url = `https://app-staging.pianote.com/api/railcontent/content?brand=pianote&sort=${sort}&statuses[]=published&limit=20&page=${page}&${type}` + filters + required_user_states
        let response = await fetch(url, {method: 'GET', headers: {Authorization: `Bearer ${auth.token}`}})        
        return await response.json()
    } catch (error) {
        console.log('Error: ', error);
        return new Error(error);
...
export async function searchContent(term, page, filtersDict) {
    ...
    if(filtersDict.topics.length > 0) {
        for(i in filtersDict.topics) {included_types = included_types + `&included_types[]=${filtersDict.topics[i]}`}
    } else {
    ...
    try {
        let auth = await getToken();
        let url = `https://app-staging.pianote.com/api/railcontent/search?brand=pianote&limit=20&statuses[]=published&sort=-score&term=${term}&page=${page}` + included_types
        let response = await fetch(url, {method: 'GET', headers: {Authorization: `Bearer ${auth.token}`}})        
        return await response.json()
    } catch (error) {
        console.log('Error: ', error);
...
export async function getMyListContent(page, filtersDict) {
    ...
    try {
        let auth = await getToken();
        var url = `https://app-staging.pianote.com/api/railcontent/content?brand=pianote&limit=20&statuses[]=published&sort=newest&page=${page}` + included_types
        let response = await fetch(url, {method: 'GET', headers: {Authorization: `Bearer ${auth.token}`}})        
        return await response.json()
    } catch (error) {
        console.log('Error: ', error);
...
export async function seeAllContent(page, filtersDict) {
    ...
    try {
        let auth = await getToken();
        let url = `https://app-staging.pianote.com/api/railcontent/content?brand=pianote&limit=20&statuses[]=published&sort=newest&page=${page}` + included_types
        let response = await fetch(url, {method: 'GET', headers: {Authorization: `Bearer ${auth.token}`}})        
        return await response.json()
    } catch (error) {
        console.log('Error: ', error);
...
export async function getContentById(contentID) {
    try {
        let auth = await getToken();
        let response = await fetch(`http://app-staging.pianote.com/railcontent/content/${contentID}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${auth.token}`,
            },
