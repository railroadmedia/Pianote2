...
export async function getLiveContent() {
  return (response = await commonService.tryCall({
    url: `${commonService.rootUrl}/musora-api/live-event`
  }));
}

export async function getScheduleContent() {
  return (response = await commonService.tryCall({
    url: `${commonService.rootUrl}/musora-api/schedule`
  }));
}

export async function getLiveScheduleContent() {
  return (response = await commonService.tryCall({
    url: `${commonService.rootUrl}/musora-api/live-schedule`
  }));
}
...
export async function getStartedContent(type, page, filters = '') {
  ...
  return commonService.tryCall({
    url: `${commonService.rootUrl}/musora-api/in-progress?brand=pianote&sort=-progress&statuses[]=published&limit=10&included_types[]=${type}&required_user_states[]=started&page=${page}${filters}`
  });
}
...
export async function getStudentFocusTypes() {
  return commonService.tryCall({
    url: `${commonService.rootUrl}/api/railcontent/shows`
  });
}
